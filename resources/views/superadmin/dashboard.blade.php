...
<div class="container-fluid p-4">
  ...
  <div class="row sp-greatings">
    ...
    <div class="row">
        <div class="col-lg">
            <div class="dashboard-card card p-3 pt-4 px-4 position-relative hover-effect" onclick="window.location='{{ route('report.post.management') }}';">
        ...
        <div class="col-lg">
            <div class="dashboard-card card p-3 pt-4 px-4 position-relative hover-effect" onclick="window.location='{{ route('employer.account') }}';" >
                <div class="d-flex flex-wrap">
                    <i class="fa fa-users f-40"></i>  
                    <h4 class="ml-2 mt-2 text-muted">Employer Account</h4> 
                </div>
                <h2 class="text-right pt-2 pb-3">{{$employer}}</h2>  
                <div class="bg-danger w-100 position-absolute fixed-bottom rounded-bottom" style="height: 30px;"></div>
            </div>  
        </div>
        <div class="col-lg">
            <div class="dashboard-card card p-3 pt-4 px-4 position-relative hover-effect" onclick="window.location='{{ route('applicant.accounts') }}';" >
                <div class="d-flex flex-wrap">
                    <i class="fa fa-users f-40"></i>
                    <h4 class="ml-2 mt-2 text-muted">Applicant Account</h4> 
                </div>
                <h2 class="text-right pt-2 pb-3">{{$applicant}}</h2>  
                <div class="bg-danger w-100 position-absolute fixed-bottom rounded-bottom" style="height: 30px;"></div>
            </div>  
        </div>
        @if(Auth::user()->user_role === 3)
        <div class="col-lg">
            <div class="dashboard-card card p-3 pt-4 px-4 position-relative hover-effect" onclick="window.location='{{ route('admin.accounts') }}';" >
                <div class="d-flex flex-wrap">
                    <i class="fa fa-user f-40"></i>  
                    <h4 class="ml-2 mt-2 text-muted">Admin Account</h4> 
                </div>
                <h2 class="text-right pt-2 pb-3">{{$admin}}</h2>  
                <div class="bg-danger w-100 position-absolute fixed-bottom rounded-bottom" style="height: 30px;"></div>
            </div>  
        </div>
        @endif
